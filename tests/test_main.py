...
from app.models import Unita, Pattuglia, Challenge, Completion
...
def test_create_and_complete_flow():
    ...
    client.post("/admin/pattuglie", data={"name": "Aquile", "capo_pattuglia": "Mario", "unita_id": 1}, auth=("admin", "admin"))
    
    # 3. Create Challenge (Admin)
    client.post("/admin/challenges", data={"name": "Nodi", "description": "Fai un nodo", "points": 10}, auth=("admin", "admin"))
    ...
    response = client.post("/complete", data={"pattuglia_id": 1, "challenge_id": 1}, auth=("progesplo", "punteggiometro"), follow_redirects=False)
