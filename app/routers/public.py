...
from app.models import Pattuglia, Challenge, Completion, Unita, User
from app.auth import get_authenticated_user, get_tech_user, get_unit_user
...
async def register_completion(
    ...
    existing = db.query(Completion).filter(
        Completion.pattuglia_id == pattuglia_id,
...
async def timeline_page(request: Request, db: Session = Depends(get_db), user: User = Depends(get_authenticated_user)):
    ...
    ).order_by(Completion.timestamp.desc()).limit(100).all()
